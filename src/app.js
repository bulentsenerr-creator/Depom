...
const state = {
  ...
  scanner: null,
  activeShelf: 'ALL',
  filter: { language:'', publisher:'', format:'', yearMin:'', yearMax:'', priceMin:'', priceMax:'', cover:'any', tag:'' },
...
function bind(id, event, handler){
  const el=$(id);
  if(!el){ console.warn('Eksik element:', id); return; }
...
async function boot(){
  ...
  bind('fileImport','change', async (e)=>{
  ...
  // home actions
  bind('btnScan','click', openScan);
  bind('btnFetch','click', onFetch);
  bind('btnSaveQuick','click', ()=> onSave({ closeAdd:false }));
  bind('btnOpenAdd','click', openAddSheet);

  // cover
  bind('coverUpload','change', onCoverUpload);
  bind('btnRemoveCover','click', ()=>{
  ...
  // scan modal
  bind('btnCloseScan','click', closeScan);
  ...
  // add sheet
  bind('navAdd','click', openAddSheet);
  bind('btnCloseAdd','click', closeAddSheet);
  bind('btnSave','click', ()=> onSave({ closeAdd:true }));
  $('addSheet').addEventListener('click', (e)=>{ if(e.target === $('addSheet')) closeAddSheet(); });

  // shelves + filter
  bind('btnOpenFilter','click', ()=>{ setPage('Shelves'); openFilterSheet(); });
  bind('navFilter','click', ()=>{ setPage('Shelves'); openFilterSheet(); });
  bind('btnCloseFilter','click', closeFilterSheet);
  bind('btnResetFilter','click', ()=>{ resetFilter(); renderFilterUi(); scheduleRender(); });
  $('filterSheet').addEventListener('click', (e)=>{ if(e.target === $('filterSheet')) closeFilterSheet(); });

  bind('searchInput','input', ()=> scheduleRender());

  // realtime filter inputs
  const rtIds = ['f_language','f_publisher','f_format','f_yearMin','f_yearMax','f_priceMin','f_priceMax','f_tag','f_cover','s_key','s_dir'];
  ...
  // bottom nav pages
  bind('navHome','click', ()=> setPage('Home'));
  bind('navShelves','click', ()=> setPage('Shelves'));
  bind('navSettings','click', ()=> setPage('Settings'));

  // settings
  bind('btnClear','click', async ()=>{
  ...
  // shelf fields sync
  bind('uc_shelf','input', ()=>{ $('uc_shelf_detail').value = $('uc_shelf').value; });
  bind('uc_shelf_detail','input', ()=>{ $('uc_shelf').value = $('uc_shelf_detail').value; });
...
function setPage(name){
  ...
  for(const id of ['navHome','navShelves','navFilter','navSettings']) $(id).classList.remove('active');
...
function scheduleRender(){
  ...
  state._rtTimer = setTimeout(()=> renderShelves(), 80);
}
...
function renderShelfTabs(items){
  ...
  for(const s of shelves){ box.appendChild(makeTab(s, s, counts[s] || 0)); }
...
function applyAllFilters(items){
  let out=[...items];
  if(state.activeShelf !== 'ALL') out = out.filter(x=> normalizeShelf(x.userCopy?.shelf) === state.activeShelf);
...
function renderLibraryList(list){
  ...
  for(const item of list){
    ...
    const pubDate = ed.publishedDate ? `• ${ed.publishedDate}` : '';
    const author = (ed.authors||[]).slice(0,2).join(', ');
    const sub=document.createElement('div');
    sub.className='sub';
    sub.textContent = `${author || '-'} • ${ed.publisher||'-'} ${pubDate}`;
